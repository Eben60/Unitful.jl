...
If `makepublic=true`, makes the dimension name, abbreviation, and corresponding `Units` and `FreeUnits` type names public.
...
"""
    @derived_dimension(name, dims, autodocs=false, makepublic=autodocs)
...
If `autodocs == true`, docstrings will be automatically generated for these aliases.
If `makepublic=true`, makes the dimension name public.
...
"""
    @refunit(symb, abbr, name, dimension, tf, autodocs=false, makepublic=autodocs)
...
If `makepublic=true`, makes the units names public.
...
"""
    @unit(symb, abbr, name, equals, tf, autodocs=false, makepublic=autodocs)
...
If `makepublic=true`, makes the unit name public.
...
"""
    @prefixed_unit_symbols(symb, name, user_dimension, basefactor, autodocs=false, makepublic=autodocs)
...
"""
macro logunit(symb, abbr, logscale, reflevel, makepublic=false)
    quote
        $Unitful.abbr(::Level{$(logscale), $(reflevel)}) = $abbr
        const global $(symb) =
            MixedUnits{Level{$(logscale), $(reflevel)}}()
    end |> esc
end
